...
declare global {
  interface Window {
    _ns_go?: (url: string) => void;
    _ie_go?: (url: string) => void;
  }
}
...
function NetscapeBrowser() {
  ...
  const [url] = useState("http://www.netscape.com/");
  const [pageTitle, setPageTitle] = useState(
    pages["http://www.netscape.com/"].title,
  );
  ...
  const go = (u?: string) => loadPage(u || inputVal, true);
  ...
  return (
    ...
    >
      ...
      >
        {["File", "Edit", "View", "Go", "Bookmarks", "Options", "Help"].map(
          (m) => (
            <span
              key={m}
        ...
        >
          <span
            style={{
              ...
              animation: loading ? "spin 0.3s steps(4) infinite" : "none",
            }}
          >
            N
          </span>
      ...
      >
        <span
          style={{ fontSize: 12, fontWeight: "bold", whiteSpace: "nowrap" }}
        >
          Location:
        </span>
        <input
      ...
      >
        <span style={{ fontSize: 11, flex: 1, fontFamily: "Arial" }}>
          {status}
        </span>
        <div
          style={{
...
function IEBrowser() {
  ...
  const [url] = useState("http://www.msn.com/");
  const [pageTitle, setPageTitle] = useState(
    pages["http://www.msn.com/"].title,
  );
  const [loading, setLoading] = useState(false);
  ...
  useEffect(() => {
    window._ie_go = (u: string) => go(u);
    return () => {
      delete window._ie_go;
    };
  }, [hist, histIdx]);
    ...
    >
      ...
      >
        {["File", "Edit", "View", "Favorites", "Tools", "Help"].map((m) => (
          <span key={m} style={{ padding: "2px 6px", cursor: "pointer" }}>
            {m}
          </span>
        ))}
      </div>
      ...
      >
        ...
        <div
          style={{
            marginLeft: 4,
            background: "linear-gradient(135deg, #1040c0, #3080ff)",
            width: 34,
            height: 34,
            borderRadius: 2,
            display: "flex",
        ...
        >
          <span
            style={{
              fontSize: 18,
              filter: loading ? "brightness(1.5)" : "none",
      ...
      >
        <span style={{ fontSize: 11, flex: 1 }}>{status}</span>
        <div
          style={{
            width: 140,
            height: 12,
...
function ChromeBrowser() {
  const gmailThreads: [string, string, string, boolean][] = [
    [
      "Google Team",
      "Welcome to Gmail! Here's how to get started",
      "10:23 AM",
      true,
    ],
    ["GitHub", "[GitHub] Your pull request was merged!", "9:41 AM", true],
    ["Netflix", "New on Netflix this month", "Yesterday", false],
    ["Spotify", "Your Wrapped is ready! 🎵", "Mon", false],
  ];
        ...
        <div style="padding:8px;">
          ...
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;">
            ${[
              ...
              ["#4ecdc4", "CSS Grid Full Course", "Kevin Powell", "1.8M views"],
              [
                "#45b7d1",
                "React Tutorial 2024",
                "Traversy Media",
    ...
    "https://github.com/": {
      ...
      html: `<div style="font-family:-apple-system,Arial,sans-serif;background:#0d1117;color:#e6edf3;padding:10px;min-height:260px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
          ${[
            ["your-project", "main • Updated 2h ago", "JavaScript"],
            ["awesome-app", "dev • Updated 1d ago", "TypeScript"],
            ["dotfiles", "main • Updated 3d ago", "Shell"],
            ["portfolio", "gh-pages • Updated 1w ago", "HTML"],
          ]
            .map(
              ([n, d, l]) => `
      ...
      html: `<div style="font-family:Arial,sans-serif;background:#fff;font-size:13px;">
        ...
        <div style="padding:0 8px;">
          ...
          <div style="display:flex;align-items:center;gap:8px;padding:6px 4px;border-bottom:1px solid #f0f0f0;font-weight:${u ? "bold" : "normal"};background:${u ? "#fff" : "#f9f9f9"};">
            <div style="width:28px;height:28px;border-radius:50%;background:${u ? "#4285f4" : "#ccc"};display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;">${String(f)[0]}</div>
    ...
    >
        ...
        {tabs.map((t, i) => (
          <div
            ...
            style={{
              ...
              background: i === activeTab ? "#fff" : "#c8ccd0",
              borderRadius: "8px 8px 0 0",
              cursor: "pointer",
              maxWidth: 140,
              fontSize: 12,
              color: i === activeTab ? "#202124" : "#5f6368",
              fontWeight: i === activeTab ? 500 : 400,
              border: i === activeTab ? "1px solid #dee1e6" : "none",
              borderBottom: "none",
            }}
          >
      ...
      >
        ...
        >
          ...
          />
          <span style={{ fontSize: 13, color: "#5f6368", cursor: "pointer" }}>
            ☆
          </span>
        </div>
        <div
      ...
      <div style={{ padding: "4px 8px", fontSize: 11, color: "#5f6368" }}>
        {loading ? "Loading page…" : "Ready"}
      </div>
      {/* Page */}
      <iframe
        ref={frameRef}
...
function AIBrowser() {
  ...
  const aiResponses = {
    default:
      "I can summarize pages, answer questions about what you're reading, book flights, write emails, or just search smarter. Try asking me anything!",
    summarize:
      "**Page Summary:** This article discusses the evolution of web browsers from 1990 to present. Key milestones: Mosaic (1993) brought images, Netscape dominated (1994–1998), Chrome revolutionized speed (2008), and AI is now being embedded directly into browsers.",
    translate:
      "**Translation complete!** Page has been translated from Japanese to English. 847 words translated across 12 paragraphs.",
  ...
  const sendAI = () => {
    ...
    const key = q.includes("summar")
      ? "summarize"
      : q.includes("translat")
        ? "translate"
        : q.includes("flight") || q.includes("book")
          ? "book"
          : "default";
    setTimeout(() => {
      setAiResponse(aiResponses[key]);
      setAiLoading(false);
      setAiInput("");
    }, 1200);
  };
  ...
  useEffect(() => {
    ...
    doc.open();
    doc.write(
      `<!DOCTYPE html><html><head><style>*{box-sizing:border-box;}body{margin:0;}</style></head><body>${pages[activeTab as keyof typeof pages]}</body></html>`,
    );
    doc.close();
  }, [activeTab]);
  ...
  return (
    ...
    >
        ...
        {spaces.map((s, i) => (
          <div
            ...
            style={{
              ...
              background: i === activeSpace ? "#1a1a2e" : "transparent",
              cursor: "pointer",
              fontSize: 11,
              color: i === activeSpace ? "#a78bfa" : "rgba(255,255,255,0.4)",
              fontWeight: i === activeSpace ? 600 : 400,
            }}
          >
            {s}
          </div>
        ...
        {tabs.map((t, i) => (
          <div
            ...
            style={{
              ...
              background:
                i === activeTab ? "rgba(255,255,255,0.06)" : "transparent",
              cursor: "pointer",
              fontSize: 11,
              color: i === activeTab ? "#e0e0ff" : "rgba(255,255,255,0.35)",
              borderRadius: "6px 6px 0 0",
              borderBottom:
                i === activeTab ? "2px solid #a78bfa" : "2px solid transparent",
              whiteSpace: "nowrap",
              overflow: "hidden",
              textOverflow: "ellipsis",
              maxWidth: 120,
      ...
      <iframe
        ref={frameRef}
        style={{ width: "100%", height: 210, display: "block", border: "none" }}
        title="ai-browser"
        sandbox="allow-scripts allow-same-origin"
      />
      {/* AI Bar */}
      ...
      >
        ...
        >
          ...
          <input
            ...
            placeholder='Ask AI... "Summarize", "Translate", "Book a flight"'
            style={{
              flex: 1,
              background: "transparent",
              border: "none",
...
const eras = [
  {
    id: 1,
    year: "1994–1997",
    label: "The Netscape Era",
    accent: "#00c878",
    bg: "from-[#001a0d] to-[#000d06]",
    tagline: "The web gets a face",
    fact: "Netscape introduced JavaScript — the language that still powers every website today.",
    stat1: "80%",
    stat1l: "Peak market share",
    stat2: "~600K",
    stat2l: "Web users in 1993",
    stat3: "1st",
    stat3l: "SSL secure browsing",
    Component: NetscapeBrowser,
  },
  {
    id: 2,
    year: "1998–2006",
    label: "The IE Monopoly",
    accent: "#4f9eff",
    bg: "from-[#00080d] to-[#000508]",
    tagline: "Microsoft wins the war",
    fact: "IE6 was so dominant it killed innovation for 5 years. Developers wrote CSS hacks just to survive it.",
    stat1: "96%",
    stat1l: "IE market share (2002)",
    stat2: "2022",
    stat2l: "IE finally retired",
    stat3: "5yrs",
    stat3l: "of stagnation",
    Component: IEBrowser,
  },
  {
    id: 3,
    year: "2008–2020",
    label: "The Chrome Revolution",
    accent: "#4285f4",
    bg: "from-[#000820] to-[#000510]",
    tagline: "Speed becomes a religion",
    fact: "Chrome's V8 engine made JavaScript 10–100× faster overnight. The modern web was born.",
    stat1: "65%",
    stat1l: "Current global share",
    stat2: "3.2B",
    stat2l: "Chrome users worldwide",
    stat3: "4yrs",
    stat3l: "To reach #1",
  ...
  },
  {
    ...
    label: "The AI Era",
    accent: "#a78bfa",
    bg: "from-[#0d001a] to-[#060008]",
    tagline: "Agent, not just window",
    fact: "The next browser won't just render pages — it will read them, summarize them, and act on your behalf.",
    stat1: "70%+",
    stat1l: "Browsers on Chromium",
    stat2: "2025",
    stat2l: "AI in every browser",
    stat3: "∞",
    stat3l: "What's next",
    Component: AIBrowser,
  },
    ...
    >
          ...
          {eras.map((_, i) => (
            <button
              ...
              style={{
                ...
                border: "none",
                background:
                  i === active ? era.accent : "rgba(255,255,255,0.15)",
                cursor: "pointer",
                transition: "all 0.3s",
                padding: 0,
              }}
      ...
      >
        ...
        >
          ...
          >
            {[
              [era.stat1, era.stat1l],
              [era.stat2, era.stat2l],
              [era.stat3, era.stat3l],
            ].map(([v, l], i) => (
              <div
                key={i}
          ...
          <div
            style={{
              ...
              border: `1px solid ${era.accent}20`,
          ...
          >
            <button
              ...
              style={{
                background: "transparent",
                border: `1px solid ${active === 0 ? "rgba(255,255,255,0.1)" : era.accent}`,
                color: active === 0 ? "rgba(255,255,255,0.2)" : era.accent,
                borderRadius: 8,
                padding: "10px 22px",
                fontSize: 13,
            ...
            <button
              ...
              style={{
                background:
                  active === eras.length - 1 ? "transparent" : era.accent,
                border: `1px solid ${era.accent}`,
                color:
                  active === eras.length - 1 ? "rgba(255,255,255,0.2)" : "#000",
                borderRadius: 8,
                padding: "10px 22px",
                fontSize: 13,
        ...
        >
          <div
            style={{
              borderRadius: 14,
              overflow: "hidden",
              boxShadow: `0 0 80px ${era.accent}25, 0 30px 80px rgba(0,0,0,0.6)`,
